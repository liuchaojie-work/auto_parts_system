...
<script>
    ...
    function load(currentPage, pageSize, condition) {
        $.get("payment/pageQuery",{"currentPage":currentPage, "pageSize":pageSize, "condition":condition}, function (pb) {
            ...
            for(var i = 0; i < pb.list.length; i++){
                ...
                var tr =
                    '<tr>\n' +
                    '                                        <td><input name="ids" type="checkbox" value="'+listElement.payWay+'"></td>\n' +
                    '                                        <td>'+(i+1)+'</td>\n' +
                    '                                        <td>'+listElement.payWay+'</td>\n' +
                    '                                        <td>'+listElement.payStatus+'</td>\n' +
                    '                                        <td>'+listElement.remark+'</td>\n' +
                    '                                        <td class="text-center">\n' +
                    '                                            <input type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#changePayment" onclick="findPaymentByPayWay(\''+listElement.payWay+'\')" value="修改"/>\n' +
                    '                                            <input type="button" class="btn btn-danger btn-xs" onclick="deletePaymentByPayWay(\''+listElement.payWay+'\')" value="删除"/>\n' +
                    '                                        </td>\n' +
                    '                                    </tr>';
                info_list+=tr;
            }
            $("#payment-list tbody").html(info_list);
            window.scrollTo(0,0);
        });
    }
