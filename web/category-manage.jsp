...
<script>
    ...
    function load(currentPage, pageSize, condition) {
        $.get("category/pageQuery",{"currentPage":currentPage, "pageSize":pageSize, "condition":condition}, function (pb) {
            ...
            for(var i = 0; i < pb.list.length; i++){
                ...
                var tr =
                    ...
                    '                                        <td class="text-center">\n' +
                    '                                            <input type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#changeCategory"  onclick="findCategoryByName(\''+listElement.name+'\')" value="修改"/>\n' +
                    '                                            <input type="button" class="btn btn-danger btn-xs" onclick="deleteCategoryByName(\''+listElement.name+'\')" value="删除"/>\n' +
                    '                                        </td>\n' +
                    '                                    </tr>';
                info_list+=tr;
            }
            $("#category-list tbody").html(info_list);
            window.scrollTo(0,0);
        });
    }
